...
public:
    SlowEnemy(sf::Vector2f position, float spawnDelay = 0) : position(position), spawnDelay(spawnDelay), enabled(false) {
        if (!texture.loadFromFile("./assets/enemy_type_one.png")) {
